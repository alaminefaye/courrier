...
@section('title', 'Détails Courrier Sortant')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Détails du Courrier Sortant</h5>
                    <div>
                        <a href="{{ route('courriers.sortants.index') }}" class="btn btn-secondary btn-sm">
                        ...
                        @if($courrier->statut === 'enregistre')
                            <form action="{{ route('courriers.sortants.transmettre', $courrier->id) }}" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-success btn-sm" onclick="return confirm('Transmettre ce courrier pour livraison ?')">
                        ...
                        @if($courrier->statut === 'transmis')
                            <form action="{{ route('courriers.sortants.confirmer', $courrier->id) }}" method="POST" class="d-inline">
                                ...
                                <button type="submit" class="btn btn-primary btn-sm" onclick="return confirm('Confirmer la livraison ?')">
                                    <i class="bx bx-check"></i> Confirmer Livraison
                                </button>
                            </form>
                        @endif
                        <a href="{{ route('courriers.sortants.qr.pdf', $courrier->id) }}" class="btn btn-info btn-sm" target="_blank">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <table class="table table-borderless">
                                ...
                                <tr>
                                    <th>Service d'Origine:</th>
                                    <td>{{ $courrier->provenanceService->nom ?? '-' }}</td>
                                </tr>
                                @if($courrier->provenanceUser)
                                <tr>
                                    <th>Personne d'Origine:</th>
                                    <td>{{ $courrier->provenanceUser->name }}</td>
                                </tr>
                                @endif
                                <tr>
                                    <th>Personne Livreur:</th>
